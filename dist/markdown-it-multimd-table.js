...
(function(global, factory) {
  ...
  function table_caption(state, silent, options, line) {
    ...
    /* A non-greedy qualifier allows the label to be matched */    const capRE = /^\[(.+?)\](\[([^[\]]+)\])?\s*$/;
 ...
 /* where backslash should be */      meta.multiline = state.src.charCodeAt(pos) === 92 /* \ */;
  ...
  function table(options, state, startLine, endLine, silent) {
     ...
     * generated here.
     */    const tableToken = new state.Token("table_open", "table", 1);
    tableToken.meta = {
      sep: null,
      cap: null,
      tr: []
    ...
    tableDFA.set_match_alphabets({
      65536: table_caption.bind(this, state, true, options),
      4096: table_separator.bind(this, state, true),
      256: table_row.bind(this, state, true, options),
      16: table_row.bind(this, state, true, options),
    ...
    /* Don't mix up DFA `_state` and markdown-it `state` */    tableDFA.set_actions((function(_line, _state, _type) {
      ...
      switch (_type) {
       case 65536:
        ...
        tableToken.meta.cap = table_caption(state, false, options, _line);
 ...
 // previously assigned at case 0x00110
       ...
       case 16:
        ...
        trToken.meta = table_row(state, false, options, _line);
 ...
 // false under headerless corner case
    ...
    if (tableToken.meta.cap) {
      ...
      const attrs = [];
      const capSide = tableToken.meta.cap.first ? "top" : "bottom";
 ...
 // array ref
  ...
  function multimd_table_plugin(md, options) {
    ...
    md.block.ruler.at("table", table.bind(null, opts), {
      alt: [ "paragraph", "reference" ]
    });
  }
  return multimd_table_plugin;
}));
