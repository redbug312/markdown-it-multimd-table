...
function table (options, state, startLine, endLine, silent) {
  ...
  for (let line = startLine; line < endLine; line++) {
    const matches = edge => {
      switch (edge) {
        case Edge(0b10000):
          return parseTableCaption(state, line, true, true)
        case Edge(0b01000):
          return table_delimiter(state, true, line)
        case Edge(0b00100):
    ...
    switch (edge) {
      case Edge(0b10000):
        ...
        cap = parseTableCaption(state, line, false, line === startLine)
      ...
      case Edge(0b01000):
        metadata.clearHasTableRowBelow()
        del = table_delimiter(state, false, line)
  ...
  if (metadata.caption) {
    ...
    if (caption.label) {
      ...
      let value = state.src.slice(start, end)
      token.attrs.push(['id', `${value}`])
    } else if (options.autolabel) {
      const [start, end] = caption.text
      let value = slugify(state.src.slice(start, end))
      token.attrs.push(['id', `${value}`])
    }
    if (side !== 'top') {
 ...
 */
function parseTableCaption (state, line, silent, atTableStart) {
  const head = state.bMarks[line] + state.blkIndent
