 ...
 */
function parseTableRow (md, line, silent, isHeader, opts) {
  const head = md.bMarks[line] + md.blkIndent
  const tail = md.eMarks[line]
  const endsWithBackslash = md.src.at(tail - 1) === '\\'
  const isContinuing = opts.multiline && endsWithBackslash
  const end = md.skipSpacesBack(tail, head) - Number(isContinuing)

  // TODO panic when the line contains only one backslash?
  const cells = scanLineCells(md, head, end)
  if (cells.length === 0) {
    return false
  } else if (silent) {
    return true
