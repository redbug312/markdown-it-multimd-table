...
function table_caption (state, silent, options, line) {
  const meta = { text: null, label: null }
  const start = state.bMarks[line] + state.sCount[line]
...
function table (options, state, startLine, endLine, silent) {
  ...
  let grp = 0x10; let mtr = -1
  let token; let trToken
  let colspan; let leftToken
  let rowspan; let upTokens = []
  let tableLines; let tgroupLines
  let tag; let text; let range; let r; let c; let b; let t
  let blockState
  ...
  if (options.headerless) {
    ...
    trToken.meta = {}  // avoid trToken.meta.grp throws exception
  }
